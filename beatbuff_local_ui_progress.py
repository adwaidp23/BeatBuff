"""
BeatBuff - Local Enhanced UI + Progress Bar 🎧
Gesture-controlled local music player with HUD, album art, volume bar, and playback timer.
"""
...
if not playlist:
    raise SystemExit("❌ No songs found. Add your local files inside /songs/<genre>/")
...
def draw_volume_and_progress(img, vol, player):
    h, w, _ = img.shape
    # Volume bar
    bar_x, bar_y, bar_w, bar_h = 20, h-80, 400, 20
    ...
    cv2.putText(img, f"Volume: {vol}%", (bar_x, bar_y - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

    # Progress bar
    length = player.get_length()   # milliseconds, -1 if unknown
    current = player.get_time()    # milliseconds
    # Protect against invalid values
    if length is None or length <= 0 or current is None or current < 0:
        progress_ratio = 0.0
    else:
        # clamp ratio between 0 and 1
        progress_ratio = max(0.0, min(float(current) / float(length), 1.0))

    prog_x, prog_y, prog_w, prog_h = 20, h - 40, w - 40, 10
    fill_w = int(progress_ratio * prog_w)
    cv2.rectangle(img, (prog_x, prog_y), (prog_x + prog_w, prog_y + prog_h), (60, 60, 60), -1)
    cv2.rectangle(img, (prog_x, prog_y), (prog_x + fill_w, prog_y + prog_h), (0, 180, 255), -1)

    # Time labels (format_time should exist in your script)
    t_text = f"{format_time(current)} / {format_time(length)}"
    # Place timer near right side of progress bar
    cv2.putText(img, t_text, (w - 220, prog_y - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
while True:
    ...
    if gesture != last_gesture:
        ...
        if gesture == "open":
            player.play(); banner_text = "▶ Playing"
        elif gesture == "fist":
            player.pause(); banner_text = "⏸ Paused"
        elif gesture == "next":
            play_next(); banner_text = "⏭ Next Track"
        elif gesture == "prev":
            play_prev(); banner_text = "⏮ Previous Track"
        elif gesture == "vol_up":
